...
Decompose plan.md into granular, independently-checkable tasks in build order. Each task:
`- [ ] T{NN} — {imperative title}` followed by: the files it touches (from Files Affected),
its acceptance signal (how you know it is done / which test covers it), and any task it depends on
(`depends: T{NN}`). Group by the Implementation Sequence phases. Tasks must collectively cover
every row of Files Affected and every story this project covers — no orphan files, no uncovered AC.
