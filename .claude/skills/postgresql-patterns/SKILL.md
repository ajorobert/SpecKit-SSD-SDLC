...
Within a single service, PostgreSQL is accessed differently for the write side vs the read side. See `backend-feature-patterns` for the marker interfaces and handler split.
...
| **Read (search-shaped)** | — | — | Goes to **Elasticsearch** via `I{Entity}SearchRepository` (see `backend-feature-patterns` Read side and `elasticsearch-patterns`), never PostgreSQL. |
...
* Repository interfaces are split per `backend-feature-patterns`: `I{Aggregate}WriteRepository` (EF Core impl) vs `I{Entity}ReadRepository` (Dapper impl).
...
**Bridging `IUserContext` → PostgreSQL session GUC** — use a `DbConnectionInterceptor` so RLS sees the tenant on every connection, including ones returned from the Npgsql pool:

```csharp
public sealed class TenantConnectionInterceptor(IUserContext tenant) : DbConnectionInterceptor
{
