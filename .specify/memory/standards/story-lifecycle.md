...
- `{ID}` — the zero-padded sequence **number only**, e.g. `001` (NOT `STORY-001`). IDs are
  global and monotonically increasing across the whole `specs/` tree.
- `active_story_id` — the full id **including** the `STORY-` prefix, e.g. `STORY-001`
  (this is what is stored in `session.yaml`). It already contains `STORY-`; never prepend a
  second one. Equivalently: `active_story_id = "STORY-" + {ID}`.
- **Folder name** = `{active_story_id}-{feature-name}` = `STORY-{ID}-{feature-name}`. The literal
  `STORY-` appears exactly once. Example: `STORY-001-customer-login`.
- `{feature-name}` — branch-friendly kebab slug of the story title
  (lowercase, spaces → hyphens, strip punctuation). Example: `customer-login`.
...
`active_story_id` already includes the `STORY-` prefix, so `STORY_DIR` interpolates it directly —
do NOT write `specs/STORY-{active_story_id}-...` (that double-prefixes to `STORY-STORY-001`).

Resolve the active story in this order:
1. `session.yaml` → `story_dir` (root-relative path written by `sk.session` / `sk.story`).
   If present, use it verbatim.
2. Else `session.yaml` → `active_story_id` (e.g. `STORY-001`): glob `specs/{active_story_id}-*/`
   and take the single match.
...
zero-pad to 3 digits → that is `{ID}`; `active_story_id = "STORY-" + {ID}`. If none exist, start
at `001`. (Also scan legacy `specs/intents/**/story-*.md` ids so a migrated repo never reuses a
number.)
...
- **Branch name:** `feature/{active_story_id}/{feature-name}-{YYYYMMDD}`
  - Example: `feature/STORY-001/customer-login-20260624`
  - Keep `{active_story_id}` verbatim (preserve `STORY-NNN` casing so it round-trips on
    `sk.session restore`). Sanitize only `{feature-name}`: lowercase, spaces → hyphens, strip
    punctuation.
